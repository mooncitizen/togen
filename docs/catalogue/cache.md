...
Cost: `togen cost` prices a cache as one meter from the bundled Redis Cache retail prices. Small is Basic C0: 730 hours a month of its `C0 Cache` meter under `Azure Redis Cache Basic`. Medium and large are Standard C1 and C3, and they are priced as two nodes, 1,460 hours of the per-node `C1 Cache Instance` or `C3 Cache Instance` meter under `Azure Redis Cache Standard`, because the whole-cache `Cn Cache` meter is missing from some regions and the per-node one is listed everywhere. That is the same total either way: the whole-cache rate is twice the node's. The Redis version does not change the price. A cache takes no usage keys, so an entry for it in the `usage` block fails validation. Backups are off and the cache answers on its public hostname, so nothing is left unpriced beyond the data transfer the catalogue README rules out. The matcher is `internal/resolve/azure/cost.go`, beside the size table's triples, so a change to the sizes and its price consequence are one diff.
