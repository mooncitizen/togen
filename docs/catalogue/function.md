...
Cost: `togen cost` prices a function on two usage keys, `invocations` (a rate) and `durationMs`. Lambda charges nothing for a function that is not invoked, so a function with no usage entry has a zero subtotal with the line `priced on defaults, no usage set` under it and `requests` and `duration` under not priced. With an entry, `invocations` a month becomes the `requests` line, and `invocations` times `durationMs` times the size's memory in GB becomes the `duration` line in GB-seconds (2M/month at 300 ms on the small 512 MB size is 300,000 GB-s), which is why the summary shows the memory alongside the runtime; `durationMs` on its own prices nothing, and `duration` stays not priced until `invocations` is there too. The two meters are the `AWS-Lambda-Requests` and `AWS-Lambda-Duration` groups of the `AWSLambda` offer file, on demand, in the project's region, for x86_64, which is what Lambda runs when `architectures` is not set; the arm64 meters are separate and a fifth cheaper. The duration meter is tiered and the snapshot holds the first tier's rate with the point where it ends (6,000,000,000 GB-s); a quantity past that is priced at the first tier's rate and the line says so. Ephemeral storage beyond the default, provisioned concurrency and the log group's ingestion are not priced. The matcher is `internal/resolve/aws/cost.go`, beside the memory table.
...
A `routes` edge from the gateway opens the function to the internet and outputs its URL, described under [gateway](gateway.md). A `reads` or `writes` edge to a database sets the connection details on the function and puts it on the connector, described under [database](database.md). A `calls` edge in either direction grants `roles/run.invoker` on the target's Cloud Run service to the caller's account and sets `<TARGET>_URL`, described under [service](service.md). A `publishes` or `consumes` edge grants the topic role or creates the Eventarc trigger, described under [queue](queue.md); a `reads` or `writes` edge to a bucket grants the object role, described under [bucket](bucket.md); either edge to a cache sets the host and port and puts the function on the connector, described under [cache](cache.md).

Cost: `togen cost` prices a function on the same two usage keys as AWS, `invocations` (a rate) and `durationMs`. A 2nd gen function is a Cloud Run service billed per request, so it costs nothing at rest: with no usage entry the subtotal is zero, with `priced on defaults, no usage set` under it and its three meters under not priced. With an entry, `invocations` a month becomes the `invocations` line on the global `Cloud Run Functions Invocations` meter, and `invocations` times `durationMs` gives the seconds the runs take, priced twice: `cpu` in vCPU-seconds at the vCPUs the size's memory comes with (0.333 at 512 MB, 0.583 at 1 GB, 1 at 2 GB), and `memory` in GB-seconds at that memory. Both are per-second meters named for the region. The min-instance meters, which price a function held warm, and the 1st gen meters are separate and not used. The matcher is `internal/resolve/gcp/cost.go`, beside the memory table.
## Azure
...
Cost: `togen cost` prices a function on two usage keys, `invocations` (a rate) and `durationMs`. The consumption plan charges nothing for a function that is not invoked, so a function with no usage entry has a zero subtotal with the line `priced on defaults, no usage set` under it and `executions` and `duration` under not priced. With an entry, `invocations` a month becomes the `executions` line on `Standard Total Executions`, and `invocations` times `durationMs` times a memory figure becomes the `duration` line in GB-seconds on `Standard Execution Time`, both under the `Functions` product on the `Standard` sku, which is what the API calls the consumption plan. The memory is the catch: `size` maps to nothing in the generated Terraform, because Y1 has one tier and scales by instance count, and the plan bills the memory a run actually observes, which no property can say. So the matcher prices small at 512 MB, medium at 1024 and large at 1536, plausible footprints under the plan's 1.5 GB ceiling, and the summary shows the figure it used. A function that holds more or less than that costs more or less than the line says. `durationMs` on its own prices nothing, and `duration` stays not priced until `invocations` is there too. The monthly grant every subscription gets (1 million executions and 400,000 GB-seconds) is a free tier and is not applied. The storage account the plan runs its content share from, and its transactions, are not priced. The matcher is `internal/resolve/azure/cost.go`, beside the memory table.
