...
A `reads` or `writes` edge from a function sets the app settings `<NAME>_HOST` (the server's FQDN), `<NAME>_PORT`, `<NAME>_NAME`, `<NAME>_USER` and `<NAME>_PASSWORD` on the function app and marks it as needing the network. The consumption plan has no virtual network integration and the private DNS zone only answers from inside the network, so a Y1 function app carries the right settings but cannot reach the server until it moves to a plan that joins the network. A service will join it (the Container Apps environment sits on the `apps` subnet), and the same edge wires a service handle the same way when the service node lands.
