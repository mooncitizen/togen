...
`togen cost` estimates the monthly bill of `togen/project.json` without touching the network. Each node type's file has a Cost paragraph per provider saying which meters it is priced on and what is left out; a node type without one is listed under `not priced` in the output rather than priced at zero. `togen cost --json` prints the same estimate as a document for the studio, and `GET /api/cost` serves it. The studio shows it too: a pill in the top bar carries the total and opens the same table as a panel on the right, every card carries its subtotal (a dash when the node is not priced), and the number follows each change to the project.
