...
Cost: `togen cost` prices a bucket on three usage keys, `storageGb`, `egressGb` and `requests` (a rate). Blob storage charges nothing for an account that holds nothing, so a bucket with no usage entry has a zero subtotal with the line `priced on defaults, no usage set` under it and its meters under not priced. With an entry, `storageGb` is the `storage` line in GB-months, `egressGb` the `egress` line, and `requests` is split a tenth writes and nine tenths reads, since one figure has to cover two meters and most buckets are read far more than written: the lines are labelled `put requests (1 in 10)` and `get requests (9 in 10)` so the split is on the page. Versioning and public access change nothing on their own; old versions are storage like any other. The meters are the hot tier of `General Block Blob v2` on the `Hot LRS` sku, which is what the account writes: `Hot LRS Data Stored`, `Hot LRS Write Operations` and `Hot Read Operations`, plus internet egress from the `Bandwidth` service's `Standard Data Transfer Out` meter for the default routing preference (Microsoft's global network). Storage and egress are tiered and the snapshot holds the first tier's rate with the point where it ends (51,200 GB-months, about 10,000 GB); a quantity past that is priced at the first tier's rate and the line says so. The 100 GB a month of free egress is a row of its own and is not applied. List and other operations, and the transactions versioning adds, are on meters of their own and are not counted. The matcher is `internal/resolve/azure/cost.go`.
