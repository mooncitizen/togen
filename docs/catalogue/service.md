...
Cost: `togen cost` prices a service on the Fargate meters from the bundled ECS list prices: vCPU-hours and GB-hours for the size's cpu and memory pair, times `minReplicas`, over a 730 hour month, since the service sits at `minReplicas` until autoscaling arrives. The task definition sets no `runtime_platform`, so the tasks run on x86 and those are the rates used, not the cheaper ARM ones. A public service adds 730 hours of its application load balancer to the same block. A private service that a gateway routes to gets the same load balancer, but the node alone cannot tell, so its hours appear as a separate `implicit ALB` block named after the service, beside the network. A private service nobody routes to has no load balancer line, which is the point of not giving it one. The one usage key is `egressGb`, the gigabytes the service sends to the internet a month: it becomes an `egress` line from the `AWSDataTransfer` offer file (`AWS Outbound` to `External`, priced from the region, tiered at 10,240 GB), and, on whichever load balancer the service has, a `capacity units` line at one LCU-hour per GB, because an LCU is the largest of four dimensions an hour (new connections, active connections, processed bytes at 1 GB an hour, rule evaluations) and processed bytes is the only one a design can say anything about, so a connection-heavy service costs more than the line says. Without an entry the service is on defaults, `priced on defaults, no usage set`, with `egress` and `capacity units` under not priced; the fixed lines stay. The first node that needs the VPC also brings the NAT gateway's hourly charge under `network`, and `natGb` under the `network` usage key prices the data it processes. The matcher is `internal/resolve/aws/cost.go`, beside the size table, so a change to the pairs and its price consequence are one diff.
