...
`minReplicas` may be zero. On GCP and Azure that means the service scales to nothing between requests and costs nothing while idle; on AWS it means no task runs until the count is raised, since Fargate does not start one on demand. Leaving it unset means one.
...
- `aws_ecs_service` with `launch_type = "FARGATE"`, `desired_count` from `minReplicas`, running on the private subnets with no public IP. A `minReplicas` of zero gives `desired_count = 0`: the service exists, its load balancer if any answers with no targets, and nothing runs until the count is raised.
...
Cost: `togen cost` prices a service on the Fargate meters from the bundled ECS list prices: vCPU-hours and GB-hours for the size's cpu and memory pair, times `minReplicas`, over a 730 hour month, since the service sits at `minReplicas` until autoscaling arrives. The task definition sets no `runtime_platform`, so the tasks run on x86 and those are the rates used, not the cheaper ARM ones. A public service adds 730 hours of its application load balancer to the same block. A private service that a gateway routes to gets the same load balancer, but the node alone cannot tell, so its hours appear as a separate `implicit ALB` block named after the service, beside the network. A private service nobody routes to has no load balancer line, which is the point of not giving it one. A service takes `egressGb` and `requests`, and AWS prices the first of them: Fargate has no request meter, so `requests` validates and prices nothing here. `egressGb` is the gigabytes the service sends to the internet a month: it becomes an `egress` line from the `AWSDataTransfer` offer file (`AWS Outbound` to `External`, priced from the region, tiered at 10,240 GB), and, on whichever load balancer the service has, a `capacity units` line at one LCU-hour per GB, because an LCU is the largest of four dimensions an hour (new connections, active connections, processed bytes at 1 GB an hour, rule evaluations) and processed bytes is the only one a design can say anything about, so a connection-heavy service costs more than the line says. Without an entry the service is on defaults, `priced on defaults, no usage set`, with `egress` and `capacity units` under not priced; the fixed lines stay. An entry that sets `requests` alone takes the defaults note away without pricing anything, since the note follows the entry rather than the lines. The first node that needs the VPC also brings the NAT gateway's hourly charge under `network`, and `natGb` under the `network` usage key prices the data it processes. The matcher is `internal/resolve/aws/cost.go`, beside the size table, so a change to the pairs and its price consequence are one diff.
...
- If any edge gives the service access to something on the private network, the template gains a `vpc_access` block naming the implicit network's connector with `PRIVATE_RANGES_ONLY` egress, and the first node that needs the network creates it: the VPC, a subnet with Private Google Access on, the connector, the reserved range and the peering. A `reads` or `writes` edge to a database does that. A `calls` edge to a private service does too, with `ALL_TRAFFIC` egress instead, and the first such caller adds a Cloud Router and Cloud NAT to the network so it keeps the internet; the NAT gateway's hourly price is not yet priced on GCP. A service with no such edge stays off the connector.
...
The target has to count the request as internal, and Cloud Run only does that when the caller sends all of its egress through the VPC. So a `calls` edge to a private service marks the caller the way a database edge does, and on top of the connector it gets `ALL_TRAFFIC` egress (`vpc_access.egress` on a service, `vpc_connector_egress_settings` on a function) rather than `PRIVATE_RANGES_ONLY`. `run.app` resolves to public addresses, so private-ranges egress would bypass the VPC and be refused at the ingress; the subnet has Private Google Access on so the private path still reaches Google APIs and Cloud Run, and the first caller with all-traffic egress adds a Cloud Router and Cloud NAT to the implicit network so it keeps the internet. A caller of a public service or of a function is not marked: a function's Cloud Run service allows all ingress, and a public one answers anyone with a valid token. The NAT gateway's hourly price is not yet priced on GCP; `togen cost` will carry it once GCP prices are bundled. The URL is built as `https://<project>-<environment>-<name>-<project number>.<region>.run.app`, with the project number from `data.google_project.current`, rather than read from the target, so two services that call each other are not a Terraform cycle.
...
A `calls` edge to a service sets `<TARGET>_URL` on the caller, `https://` plus the target's ingress FQDN. For a private service that is the internal FQDN, which a container app in the same environment resolves and a function app on the consumption plan does not, so a function calling a private service is marked as needing the network the same way the database edge marks it. A public service is called by its public FQDN. A `calls` edge to a function sets `<TARGET>_URL` to `https://` plus the function app's default hostname. Nothing else is created: everything on Azure answers HTTPS on its own name. The hostnames are built from the names Togen chose, `<name>.azurewebsites.net` for a function app and `<name>.<default domain>` or `<name>.internal.<default domain>` of the Container Apps environment for a service, so a caller never references the target's resource and two services can call each other.

Cost: `togen cost` prices a service on the Container Apps Consumption meters: `Standard vCPU Active Usage` and `Standard Memory Active Usage`, the size's cpu and memory pair times `minReplicas`, over a 730 hour month. The API prices both per second and the refresh converts them to the hour. `minReplicas` replicas are priced active for the whole month, which is the rate a replica serving requests pays; Azure bills a replica that is up but idle on the cheaper idle meters, so a quiet service costs less than the line says, and `min_replicas = 0` prices at zero. The environment, its ingress and its HTTPS certificate have no charge of their own on the Consumption profile, so there is no load balancer line as on AWS. The two usage keys are `requests`, which becomes a `requests` line on `Standard Requests`, and `egressGb`, which becomes an `egress` line on the `Bandwidth` `Standard Data Transfer Out` meter for the default routing preference (Microsoft's global network), tiered at about 10,000 GB a month. Without an entry the service is on defaults, `priced on defaults, no usage set`, with `requests` and `egress` under not priced; the vCPU and memory lines stay. The monthly grant every subscription gets (180,000 vCPU-seconds, 360,000 GiB-seconds and 2 million requests) is a free tier and is not applied, so a small service reads about five dollars dearer than a first one on an empty subscription. The Log Analytics workspace's ingestion and retention, and pulls from a registry, are not priced. The matcher is `internal/resolve/azure/cost.go`, beside the size table, so a change to the pairs and its price consequence are one diff.
