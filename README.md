...
`examples/gcp-full` is the same sketch as `aws-full` on Google Cloud: the same nodes, properties and edges, so what one provider does with a pairing can be read beside the other. Two edges bend to Cloud Run: the `admin` service is `public`, because a private Cloud Run service refuses a call from another service until caller egress goes through the VPC, and `worker` calls `mailer` rather than the other way round, because a `calls` loop is a reference cycle Terraform refuses. `just acceptance` validates it with the `google` and `random` providers, and `togen cost` lists every node under not priced until GCP prices are bundled.
