...
`examples/azure-basic` is the first Azure sketch: a gateway routing to a function, which becomes a resource group, a Linux Function App on a consumption plan with its storage account, and an output carrying the app's URL. `just acceptance` validates it with the `azurerm` provider alongside the AWS examples, and it grows as the Azure resolver does. `examples/gcp-basic` is the same idea on Google Cloud, cut down to what the GCP resolver supports so far: a gateway routing to a function that reads a Cloud SQL database over the private network. The gateway creates nothing there, since the function's Cloud Run URL is the public entry point, so the outputs are `api_orders_url` and `orders_db_connection_name`. `just acceptance` generates and validates it with the AWS examples, and `terraform init` fetches the google and random providers into the same plugin cache.
