...
`togen/layout.json` is version 2, with the positions and viewport of each view under `views` by view id. A version 1 file is read as the overview and written back as version 2 on the studio's next save. The API serves both at `/api/views`.
...
![The view editor, renaming a view and choosing which nodes it shows](/togen/images/view-editor.png)
